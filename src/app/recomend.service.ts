...
export class RecomendService {
  ...
  addRecomendation(author: string, laptop_size: string, brand:string, laptop:string, comment:string){
    let id;
    ...
    return this.authService.userID.pipe(
      take(1),
      switchMap(userID=>{
        newRecomend= new RecomendModel(null,laptop, brand, laptop_size, userID, comment);
        return this.authService.token;}), take(1), switchMap((token)=>{
        return this.http.post<{name:string}>('https://mobile-racunarstvo-default-rtdb.firebaseio.com/recomendation.json?auth='+token,newRecomend);
  ...
  getRecomendations(){
    ...
    return this.authService.userID.pipe(
      ...
      switchMap(userID=>{
        userLogged=userID;
        return this.authService.token;}), take(1), switchMap((token)=>{
        return this.http.get<{[key: string]: RecomendData}>('https://mobile-racunarstvo-default-rtdb.firebaseio.com/recomendation.json?auth='+token);}),
        take(1), switchMap((recomendationData)=>
        {
          ...
          for(const key in recomendationData){
            if(recomendationData.hasOwnProperty(key) && userLogged!=null && userLogged!=recomendationData[key].author){
              recomends.push(new RecomendModel(key,recomendationData[key].laptop, recomendationData[key].brand,
                recomendationData[key].laptop_size, recomendationData[key].author, recomendationData[key].comment));
            }
          }
          ...
          for(const key in recomendationData){
            if(recomendationData.hasOwnProperty(key) && userLogged!=null && userLogged==recomendationData[key].author){
              recomends.push(new RecomendModel(
                key,recomendationData[key].laptop, 
                recomendationData[key].brand,
                recomendationData[key].laptop_size, 
  ...
  getStarredReccomendation(){
    ...
    return this.authService.userID.pipe(
      ...
      switchMap(userID=>{
        userLogged=userID;
        return this.authService.token;}), take(1), switchMap((token)=>{
        return this.http.get<{name:string}>('https://mobile-racunarstvo-default-rtdb.firebaseio.com/staredlaptops.json?auth='+token);
  ...
  addStaredRecomendation(recomend: RecomendModel){
        ...
        return this.authService.token.pipe(take(1), switchMap((token)=>{
          return this.http.post<{name:string}>('https://mobile-racunarstvo-default-rtdb.firebaseio.com/staredlaptops.json?auth='+token,stared)}), take(1),
  ...
  getStaredRecomendations(){
    ...
    return this.authService.userID.pipe(
      ...
      switchMap(userID=>{
        userLogged=userID;
        return this.authService.token;}), take(1), switchMap((token)=>{
        return this.http.get<{key:string}>('https://mobile-racunarstvo-default-rtdb.firebaseio.com/staredlaptops.json?auth='+token);
  ...
  editRecomendation(id: string, author: string, laptop_size: string, brand:string, laptop:string, comment:string){
    var index:number;
    let idd;
    let newRecomend: RecomendModel;
    return this.authService.userID.pipe(
      ...
      switchMap(userID=>{
        idd=userID;
        newRecomend= new RecomendModel(id,laptop, brand, laptop_size, userID, comment);
        return this.authService.token;}), take(1), switchMap((token)=>{
        return this.http.put<{name:string}>('https://mobile-racunarstvo-default-rtdb.firebaseio.com/recomendation/'+id+'/.json?auth='+token,newRecomend)
        ...
        updatedRec[index]={id:id, laptop:laptop,brand: brand, laptop_size:laptop_size, author: idd, comment:comment};
        this.myRecomendation.next(updatedRec);
      })
    ); 
        
  }
  deleteStared(stared: StaredModel){
    var index:number;
    return this.authService.token.pipe(take(1), switchMap((token) =>{
      return this.http.delete<{name:string}>('https://mobile-racunarstvo-default-rtdb.firebaseio.com/staredlaptops/'+stared.id+'.json?auth='+token)
    }), switchMap((recData) =>{
  ...
  deleteReccomendation(rec:RecomendModel){
    var index:number;
      return this.authService.token.pipe(take(1), switchMap((token) =>{
        return this.http.delete<{name:string}>('https://mobile-racunarstvo-default-rtdb.firebaseio.com/recomendation/'+rec.id+'.json?auth='+token)
      }), switchMap((recData) =>{
        return this.myRecomendation;
